...
for i in range(max_devs):
    ...
    for k in list(devinfo.items()):
        ...
        if name == 'hostApi':
            value = f"{str(value)} ({p.get_host_api_info_by_index(k[1])['name']})"
...
try:
    ...
    print(f"No Input devices: {e[0]}")

try:
    def_index = p.get_default_output_device_info()['index']
    print(f"Default Output Device: {def_index}")
    devinfo = p.get_device_info_by_index(def_index)
    for k in list(devinfo.items()):
        name, value = k
        if name == 'hostApi':
